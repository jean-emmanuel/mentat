        ...
        if name not in self.states and preload:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(f'failed to parse state file "{file}"', exception=True)
                finally:
                    f.close()

            except Exception as e:
                self.logger.error(f'failed to open state file "{file}"', exception=True)
        ...
        if not preload:
            ...
            if name in self.states:
                try:
                    ...
                    self.logger.error(f'failed to load state "{name}"', exception=True)
            else:
        ...
        **Parameters**
        ...
        if name in self.states:
            ...
            try:
                ...
                self.logger.error(f'failed to delete state file "{self.states_folder}/{name}.json"', exception=True)
        else:
            self.logger.error(f'state "{name}" not found')
